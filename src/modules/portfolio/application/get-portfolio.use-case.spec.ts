...
describe('GetPortfolioUseCase', () => {
    it('returns valued positions without account identity or wallet addresses', async () => {
        ...
        const assets = {
            getAssets: jest
                .fn()
                .mockResolvedValue([{ assetId: 'near', priceUsd: '3.5', priceUpdatedAt: '2026-08-19T11:59:00Z' }]),
        };
        ...
        expect(result.positions[0]).toEqual(
            expect.objectContaining({ symbol: 'NEAR', valueUsd: '7', allocationPercent: '100.00' }),
        );
